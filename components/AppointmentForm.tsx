            ...
            <div className="fixed inset-0 bg-brand-950/80 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm sm:p-6 overflow-y-auto">
                ...
                >
                    <div className="p-5 sm:p-8 text-center bg-gradient-to-br from-brand-50 to-white">
                        <button onClick={onClose} className="absolute top-3.5 right-3.5 text-slate-400 hover:text-brand-900 transition-colors p-1.5 hover:bg-brand-50 rounded-full">
                            <CloseIcon />
                        </button>
            ...
            >
                <div className="sticky top-0 bg-white/95 backdrop-blur-sm px-4 py-3 sm:px-6 sm:py-4 md:px-8 md:py-6 border-b border-brand-100 flex justify-between items-center z-20">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                        <div className="w-7 h-7 sm:w-10 sm:h-10 text-brand-600 bg-brand-50 p-1 sm:p-2 rounded-full"><AppointmentIcon /></div>
                        <h2 className="text-lg sm:text-2xl md:text-3xl font-extrabold text-brand-900">Agendar Cita</h2>
                    </div>
                    <button onClick={onClose} className="text-slate-400 hover:text-brand-900 transition-colors p-1 hover:bg-brand-50 rounded-full">
                        <CloseIcon />
                    </button>
                </div>

                <div className="flex-1 overflow-y-auto p-3.5 sm:p-6 md:p-8 space-y-4 sm:space-y-6 md:space-y-8">
                     <motion.div
                         initial={{ x: -20, opacity: 0 }}
                         animate={{ x: 0, opacity: 1 }}
                         transition={{ delay: 0.1 }}
                     >
                        <h3 className="text-sm sm:text-xl font-bold text-brand-700 mb-3 sm:mb-6 flex items-center border-b border-brand-100 pb-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-3 sm:gap-y-6">
                            {/* Name Field */}
                            <div>
                                <label htmlFor="name" className="block text-[11px] sm:text-sm font-semibold text-brand-800 mb-1 sm:mb-2">Nombre Completo</label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-brand-400 w-10">
                                        <UserIcon />
                                    </div>
                                    <input type="text" id="name" value={name} onChange={e => setName(e.target.value)} required className="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2 sm:py-3.5 pl-11 pr-4 text-xs sm:text-base text-brand-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all font-medium placeholder:text-slate-400" placeholder="Ej. Ana Torres" />
                                </div>
                            </div>
                            {/* Phone Field */}
                            <div>
                                <label htmlFor="phone" className="block text-[11px] sm:text-sm font-semibold text-brand-800 mb-1 sm:mb-2">Teléfono</label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-brand-400 w-10">
                                        <PhoneIcon />
                                    </div>
                                    <input type="tel" id="phone" value={phone} onChange={e => setPhone(e.target.value)} required className="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2 sm:py-3.5 pl-11 pr-4 text-xs sm:text-base text-brand-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all font-medium placeholder:text-slate-400" placeholder="Ej. 987 654 321"/>
                                </div>
                            </div>

                            {/* Service Field */}
                            <div className="md:col-span-2">
                                <label htmlFor="service" className="block text-[11px] sm:text-sm font-semibold text-brand-800 mb-1 sm:mb-2">Tipo de Servicio</label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-brand-400 w-10">
                                        <ServiceIcon />
                                    </div>
                                    <select 
                                        ...
                                        required 
                                        className="appearance-none block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 sm:py-3.5 pl-11 pr-10 text-xs sm:text-base text-brand-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all font-medium"
                                    >
                         ...
                         <div className="grid grid-cols-3 gap-2 sm:gap-4">
                            {timeSlotsByDay.map(({ date }, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        <p className={`text-[9px] sm:text-xs md:text-sm font-bold uppercase tracking-wider mb-0.5 sm:mb-1 ${isSelected ? 'text-brand-100' : 'text-slate-500'}`}>{date.toLocaleDateString('es-ES', { weekday: 'short' }).replace('.', '')}</p>
                                        <p className={`text-xl sm:text-4xl md:text-5xl font-black my-0.5 sm:my-2 ${isSelected ? 'text-white' : 'text-brand-900'}`}>{date.getDate()}</p>
                                        <p className={`text-[9px] sm:text-xs md:text-sm font-bold uppercase mt-0.5 sm:mt-1 ${isSelected ? 'text-brand-100' : 'text-slate-500'}`}>{date.toLocaleDateString('es-ES', { month: 'short' }).replace('.', '')}</p>
                                    </motion.button>
                                );
                            })}
                        </div>
                         ...
                         {selectedDate && (
                             ...
                             >
                                ...
                                {selectedDaySlots.length > 0 ? (
                                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                                        {selectedDaySlots.map(slot => (
                                            <motion.button 
                                                ...
                                                className={`py-2 px-1 rounded-lg border text-xs sm:text-sm md:text-base font-bold transition-all duration-200 ${
                                    ...
                                    <p className="text-center text-slate-500 bg-slate-100 p-3 rounded-lg text-xs">No quedan horarios disponibles para este día.</p>
                                )}
                             </motion.div>
                         )}
                     </div>
                ...
                >
                    <motion.button 
                        ...
                        className="w-full bg-brand-600 text-white px-5 sm:px-8 py-3.5 sm:py-4 rounded-xl hover:bg-brand-700 font-bold shadow-lg shadow-brand-600/20 disabled:bg-slate-300 disabled:shadow-none disabled:text-slate-500 disabled:cursor-not-allowed transition-all text-sm sm:text-lg tracking-wide"
                    >
                        Confirmar Cita
                    </motion.button>
                </motion.div>
