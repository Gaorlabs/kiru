        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                <div className="p-4 flex justify-between items-center border-b border-slate-200">
                    <h2 className="text-xl font-bold text-slate-800">{formData.id ? 'Editar Doctor' : 'Nuevo Doctor'}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        <label htmlFor="specialty" className="block text-sm font-medium text-slate-600 mb-1">Especialidad</label>
                        <div className="relative"><BriefcaseIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" /><input type="text" name="specialty" value={formData.specialty} onChange={handleChange} required className="w-full pl-10 p-2 border border-slate-300 rounded-md" /></div>
                    </div>
                
                    <div className="p-4 border-t border-slate-200 bg-slate-50 rounded-b-xl flex justify-end gap-3 -mx-6 -mb-6 mt-6">
